...
#           c(meanBas, stdeBas, -1)))
    ...
    ## Now run the HMM.
        ...
        dataList[[i]] <- geneData
			...
			## Draw an image...
			if(!is.null(prefix)) {
				pdf(paste(prefix,".",i,".",genes[i,5],".pdf",sep=""), width=6, height=6)
					par(mar=c(5, 4, 4, 4) + 0.1)
					plot((c(1:NROW(gene))*size), gene, axes=F, xlab="", ylab="", ylim=c(max(gene),min(gene)),
					...
					abline(h=median(gene[ansVitervi == 2]), col="red", lwd=1.5)
					axis(1,pretty(range(c(1:NROW(gene))*size),10))
					mtext("Position (bp)",side=1,col="black",line=2.5)
					mtext("GDNF 60-0 min", side=2, col="black", line=2.5)
					box()
				dev.off()
			}
